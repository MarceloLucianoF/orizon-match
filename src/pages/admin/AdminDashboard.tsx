  ...
  const renderOverview = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard label="Total de Usuários" value={metrics.totalUsers} icon={Users} trend={12} color="indigo" description={`${metrics.breakdownUsers.icts} ICTs / ${metrics.breakdownUsers.companies} Orgs`} />
        <StatsCard label="Projetos Ativos" value={metrics.totalProjects} icon={FolderKanban} trend={8} color="cyan" />
        <StatsCard label="Matches Gerados" value={metrics.totalMatchesGenerated} icon={Network} trend={24} color="emerald" />
        <StatsCard label="Deal Flows" value={metrics.totalActiveDeals} icon={Zap} trend={5} color="amber" />
      </div>
        ...
        <div className="bg-[#0A0514] border border-slate-800 rounded-3xl p-6 flex flex-col shadow-xl">
          ...
          <div className="mt-6 pt-6 border-t border-slate-800">
            <p className="text-[10px] text-slate-500 italic flex items-center gap-2">
              <Globe size={12} /> Heatmap baseado em logins
            </p>
          </div>
        </div>
      </div>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button onClick={() => handleUpdateSubscription(u.id, u.subscriptionStatus)} className={`px-2 py-1 rounded text-[10px] font-bold transition-all border ${u.subscriptionStatus === 'premium' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20 hover:bg-amber-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20 hover:bg-indigo-500/20'}`}>
                        {u.subscriptionStatus === 'premium' ? "Downgrade" : "Premium"}
                      </button>
    ...
    <div className="bg-[#0A0514] border border-slate-800 rounded-2xl overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {deals.length === 0 ? (
              <tr><td colSpan={3} className="p-8 text-center text-slate-500">Nenhum Deal Flow ativo na plataforma.</td></tr>
            ) : (
              deals.map(deal => (
    ...
    <div className="space-y-6 pb-12 font-sans">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-100 uppercase tracking-tight flex items-center gap-3">
              Admin Workspace <span className="bg-fuchsia-500 text-white text-[10px] px-2 py-0.5 rounded-full uppercase tracking-widest font-bold">God Mode</span>
            </h1>
            <p className="text-fuchsia-400/70 text-sm font-medium mt-1">Orizon OS Controle Central</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-px overflow-x-auto custom-scrollbar">
        {[
          ...
          { id: 'deals', label: 'Deal Flows', icon: Activity },
          { id: 'logs', label: 'Auditoria de IA', icon: FileSearch },
        ].map(tab => (
          <button
            key={tab.id}
