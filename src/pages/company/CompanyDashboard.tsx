...
const COLUMNS: { id: DealStatus; title: string; color: string }[] = [
  { id: 'triagem', title: 'Triagem', color: 'border-slate-700 bg-slate-800/30' },
  { id: 'avaliacao', title: 'Em Avaliação', color: 'border-blue-500/30 bg-blue-500/10' },
...
export function CompanyDashboard() {
  ...
  useEffect(() => {
    // Mocking deals for the CRM Kanban
    // In production, this would fetch from a 'deals' or 'conversations' collection
    setTimeout(() => {
      setDeals([
  ...
  const moveDeal = (dealId: string, newStatus: DealStatus) => {
    ...
    // Smart Prompts Logic (Precision Matching Trend)
    if (newStatus === 'due_diligence') {
      setSmartPrompt({
        ...
        action: "Enviar NDA Automático"
      });
    } else if (newStatus === 'negociacao') {
        setSmartPrompt({
            dealId,
            message: "Negociação iniciada. Deseja abrir a Sala de Dados Virtual (VDR)?",
  ...
  return (
    <div className="space-y-8 h-[calc(100vh-8rem)] flex flex-col">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Deal Flow</h1>
          <p className="text-slate-400 mt-1">Gerencie seu pipeline de investimentos e parcerias estratégicas.</p>
        </div>
        <div className="flex gap-3">
          ...
          <Link to="/matches" className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-5 py-2.5 rounded-xl font-medium transition-all border border-slate-700">
            Explorar Mercado
          </Link>
        </div>
      </div>
      ...
      {smartPrompt && (
        <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-xl p-4 flex items-center justify-between animate-in slide-in-from-top-4">
          <div className="flex items-center gap-3 text-indigo-300">
            <AlertCircle size={20} />
            <span className="font-medium">{smartPrompt.message}</span>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setSmartPrompt(null)} className="text-slate-400 hover:text-slate-200 px-3 py-1.5 text-sm">Ignorar</button>
            <button 
              onClick={() => {
                alert(`Ação "${smartPrompt.action}" simulada com sucesso!`);
                setSmartPrompt(null);
              }} 
              className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-1.5 rounded-lg text-sm font-bold shadow-[0_0_10px_rgba(79,70,229,0.3)] transition-all"
            >
              {smartPrompt.action}
            </button>
          </div>
              ...
              <div key={column.id} className={`w-80 flex flex-col rounded-2xl border ${column.color} p-4`}>
                ...
                <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-1">
                  {columnDeals.map(deal => (
                    <div key={deal.id} className="bg-slate-900 border border-slate-700/80 rounded-xl p-4 cursor-grab hover:border-indigo-500/50 transition-all shadow-lg group">
                      ...
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-xs text-slate-500 ml-1">Equipe ativa</span>
                      </div>

                      <div className="flex items-center gap-2 border-t border-slate-800 pt-3">
                         <select 
                            className="bg-slate-950 border border-slate-700 text-xs text-slate-300 rounded p-1 w-full outline-none focus:border-indigo-500"
                            value={deal.status}
                  ...
                  {columnDeals.length === 0 && (
                    <div className="h-24 border-2 border-dashed border-slate-700/50 rounded-xl flex items-center justify-center text-slate-500 text-sm">
