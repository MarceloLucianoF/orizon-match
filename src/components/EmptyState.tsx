...
export function EmptyState({ icon: Icon, title, description, ctaLabel, ctaLink, onCtaClick }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-10 md:py-16 px-6 text-center">
      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-slate-800/60 border border-slate-700/50 flex items-center justify-center mb-4 md:mb-5">
        <Icon className="text-slate-500" size={28} />
      </div>
      <h3 className="text-base md:text-lg font-semibold text-slate-300 mb-2">{title}</h3>
      <p className="text-sm text-slate-500 max-w-xs md:max-w-sm leading-relaxed mb-5 md:mb-6">{description}</p>
      {ctaLabel && ctaLink && (
        <Link
          to={ctaLink}
