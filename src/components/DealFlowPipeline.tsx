...
export function DealFlowPipeline({ currentStage }: DealFlowPipelineProps) {
  ...
  return (
    <div className="w-full bg-slate-900/80 backdrop-blur-xl border-b border-slate-800 py-4 px-8 sm:px-12">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative flex justify-between items-center w-full pb-8">
          ...
          <div className="absolute left-0 top-4 -translate-y-1/2 w-full h-1 bg-slate-800 rounded-full z-0"></div>
          
          <div 
            className="absolute left-0 top-4 -translate-y-1/2 h-1 bg-indigo-500 rounded-full z-0 transition-all duration-500"
          ...
          {STAGES.map((stage, index) => {
            ...
            return (
              <div key={stage.id} className="relative z-10 flex flex-col items-center">
                ...
                <span className={`text-[10px] sm:text-xs font-medium absolute top-10 whitespace-nowrap left-1/2 -translate-x-1/2 transition-colors ${
