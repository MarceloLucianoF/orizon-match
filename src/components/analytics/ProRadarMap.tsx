            ...
            <Marker key={hub.id} position={[hub.lat, hub.lng]}>
              <Popup className="custom-popup">
                <div className="p-3 bg-slate-900 text-slate-200 border border-slate-700 rounded-lg shadow-xl min-w-[180px]">
                  <span className="text-[10px] font-black text-teal-400 uppercase tracking-widest">{hub.type}</span>
                  ...
                  <div className="mt-3 pt-2 border-t border-slate-800 flex justify-between items-center">
                    ...
                    <button className="text-[10px] text-teal-400 hover:text-white transition font-bold uppercase">Ver Detalhes</button>
                  </div>
                </div>
              </Popup>
            </Marker>
      ...
      {!isPremium && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-slate-950/40 backdrop-blur-md p-8 text-center animate-in fade-in duration-500">
          <div className="relative mb-6">
            ...
            <div className="relative bg-slate-900 p-5 rounded-3xl border border-teal-500/30 shadow-2xl">
              <MapIcon className="w-10 h-10 text-teal-400" />
            </div>
          </div>
          
          <div className="space-y-3 max-w-sm">
            <h3 className="text-2xl font-black text-white tracking-tight flex items-center justify-center gap-3">
              Radar de Inovação <Lock className="w-5 h-5 text-teal-500" />
            </h3>
          ...
          <div className="mt-8 space-y-4">
            <button 
              ...
              className="px-8 py-4 bg-gradient-to-r from-teal-650 to-teal-500 hover:from-teal-550 hover:to-teal-400 text-white font-black text-sm uppercase tracking-widest rounded-xl shadow-[0_0_30px_rgba(0,181,156,0.3)] transition-all transform hover:scale-105 active:scale-95"
            >
              Assinar InovaHelix Pro
            </button>
